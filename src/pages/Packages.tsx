...
Purpose: Compare and display packages
Editable Sections: Package data from packages.js
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {packagesData.map((pkg, idx) => (
            ...
            >
              ...
              {/* Package Name */}
              <h3 className="text-maroon text-2xl font-serif mb-8">
                {pkg.name[language]}
              </h3>
              ...
              <Link
                to="/contact"
                className={`w-full py-4 text-center font-bold uppercase tracking-widest text-xs transition-all ${
                  pkg.featured
                    ? 'bg-gold text-white hover:bg-maroon'
                    : 'bg-transparent border border-maroon text-maroon hover:bg-maroon hover:text-white'
                }`}
              >
                {t('common.bookNow')}
              </Link>
      ...
      <section className="container mx-auto px-6 mt-20">
        <div className="bg-maroon p-12 text-center text-white rounded-sm shadow-2xl relative overflow-hidden">
          ...
          >
            ...
            <Link
              to="/contact"
              className="inline-block px-12 py-3 bg-white text-maroon font-serif text-lg hover:bg-gold hover:text-white transition-all shadow-lg italic"
            >
              {t('packages.discuss')}
            </Link>
          </motion.div>
